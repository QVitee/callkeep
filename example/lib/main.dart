...
class _MyAppState extends State<HomePage> {
  ...
  Future<void> didPerformSetMutedCallAction(
    ...
    print(
      '[didPerformSetMutedCallAction] ${event.callUUID}, '
      'number: $number (${event.muted})',
    );
  ...
  Future<void> didToggleHoldCallAction(
    ...
    final String number = calls[event.callUUID].number;
    print(
      '[didToggleHoldCallAction] ${event.callUUID}, '
      'number: $number (${event.hold})',
    );
  ...
  Future<void> updateDisplay(String callUUID) async {
    ...
    /// Workaround because Android doesn't display well displayName,
  ...
  Widget buildCallingWidgets() {
    return Column(
      ...
      children: calls.entries
          .map(
            (MapEntry<String, Call> item) => Column(
              mainAxisAlignment: MainAxisAlignment.start,
              children: [
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ElevatedButton(
                onPressed: () async {
                  displayIncomingCall('0955555555');
                },
                child: const Text('Display incoming call now'),
              ),
              ElevatedButton(
                onPressed: () async {
                  displayIncomingCallDelayed('0955555555');
                },
  ...
  Future<void> onMessageHandler(RemoteMessage message) {
    ...
    if (message.notification != null) {
      if (message.notification.body != null) {
        /// Handle data message
        var payload = jsonDecode(message.notification.body);
  ...
  Future<void> onMessageOpenedAppHandler(RemoteMessage message) {
    ...
    if (message.notification.body != null) {
      ...
      _callKeep.on(
        ...
        (CallKeepPerformEndCallAction event) {
          print('backgroundMessage: CallKeepPerformEndCallAction '
              '${event.callUUID}');
        },
      );
      if (!_callKeepInited) {
